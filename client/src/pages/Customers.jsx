...
function Customers() {
  ...
  const handleChange = (e) => {
    ...
    // Mobile number validation logic: restrict to numbers and max 10 digits
    if (name === "phone") {
                    ...
                    {filteredCustomers.map((c) => (
                      <tr key={c._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 flex gap-2 justify-end">
                          <button 
                            onClick={() => { setEditingId(c._id); setForm(c); setShowForm(true); }} 
                            className="text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded-lg text-xs font-semibold transition"
                          >
                            Edit
                          </button>
                          <button 
                            onClick={() => handleDelete(c._id)} 
                            className="text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1 rounded-lg text-xs font-semibold transition"
                          >
                            Delete
                          </button>
                        </td>
                ...
                {filteredCustomers.map((c) => (
                  <div key={c._id} className="p-5 space-y-3">
                    ...
                    <div className="flex gap-3 pt-2">
                      <button 
                        onClick={() => { setEditingId(c._id); setForm(c); setShowForm(true); }} 
                        className="flex-1 text-blue-700 bg-blue-50 py-2 rounded-lg font-bold text-sm hover:bg-blue-100 transition"
                      >
                        Edit Profile
                      </button>
                      <button 
                        onClick={() => handleDelete(c._id)} 
                        className="flex-1 text-red-700 bg-red-50 py-2 rounded-lg font-bold text-sm hover:bg-red-100 transition"
                      >
                        Delete
                      </button>
                    </div>
